...
class quality:
    ...
    def completeness_report(df, show_timing: bool = False) -> RichTable:
        ...
        table = RichTable(title="Data Completeness Report", show_header=True, header_style="bold blue")
        ...
        for col in null_counts.index:
            ...
            table.add_row(col, str(non_null), str(null_counts[col]), f"{null_pct:.1f}%", status)
    ...
    def consistency_check(df, show_timing: bool = False) -> RichTable:
        ...
        table = RichTable(title="Data Consistency Check", show_header=True, header_style="bold blue")
        table.add_column("Column", style="cyan")
    ...
    def uniqueness_analysis(df, show_timing: bool = False) -> Tuple[int, RichTable]:
        ...
        table = RichTable(title="Uniqueness Analysis", show_header=True, header_style="bold blue")
        table.add_column("Column", style="cyan")
        ...
        for col in df.columns:
            ...
            table.add_row(col, str(unique_count), f"{uniqueness_pct:.1f}%", col_type)
    ...
    def data_profile(df, show_timing: bool = False) -> Dict[str, Union[float, str, int]]:
        ...
        consistency_issues = sum(1 for col in df.columns if df[col].dtype == 'object' and df[col].apply(type).nunique() > 1)
        consistency = max(0, 100 - (consistency_issues / len(df.columns) * 50))

        uniqueness_issues = sum(1 for col in df.columns if (df[col].nunique() / len(df) * 100) > 99 or (df[col].nunique() / len(df) * 100) < 1)
        ...
        overall_score = completeness * 0.4 + consistency * 0.3 + uniqueness * 0.2 + duplicates * 0.1

        table = RichTable(title="Data Quality Profile", show_header=True, header_style="bold blue")
    ...
    def outlier_summary(df, show_timing: bool = False) -> RichTable:
        ...
        table = RichTable(title="Outlier Summary", show_header=True, header_style="bold blue")
        table.add_column("Column", style="cyan")
        ...
        for col in numeric_df.columns:
            ...
            table.add_row(col, f"{min_val:.2f}", f"{max_val:.2f}", f"{range_val:.2f}", f"{issue} ({outlier_count})")
